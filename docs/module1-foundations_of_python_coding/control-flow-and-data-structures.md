...
Once you can assign variables to values and write functions, the next step is telling Python how to make decisions, repeat work, and organize collections of data.

## Lists, tuples, sets and dictionaries

Python has several built-in data structures, each with a different purpose. More details can be found in
...
In practice, you will mainly use **lists** and **dictionaries** to control data and validations.
...
You can substitute elements in the list:
...
```python
print(samples[0], samples[2], samples[-1])
```

```
steel, silica, ceramic
```
...
```python
print(samples[1:3])  # from element in index 1 to element in index 2
```
...
```python
list(range(2, 17, 3))  # from 2 to 16 jumping every 3 integers
```
...
If you try substituting an element in a tuple:
...
There is [a very comprehensive library of methods](https://docs.python.org/3/library/stdtypes.html#string-methods) that can be used with strings and come in handy very often:
...
[Dictionaries](https://docs.python.org/3/library/stdtypes.html#mapping-types-dict) map **keys** to **values**, also called *associative arrays* in computer science. They are very powerful and widely used in Python:
...
```python
elements = {"Fe", "C", "Mn"}
"Fe" in elements  # checking if "Fe" appears in the set elements
```
...
**Why this happens:**
- A string is a sequence of characters (a tuple)
...
A very helpful tool for visualizing the recursion mechanism with your custom code: enter it the code box right-hand side and press *play*. It's source and some more example code is on [its GitHub project page](https://github.com/bterwijn/invocation_tree).
